...
function ChatThrottleLib.Hook_BNSendGameData(presenceID, prefix, text)
	if bMyTraffic then
		return
	end
	local self = ChatThrottleLib
...
function ChatThrottleLib:BNSendGameData(prio, prefix, text, bnetIDAccount, queueName, callbackFn, callbackArg)
	if not self or not prio or not prefix or not text or not self.Prio[prio] then
		error('Usage: ChatThrottleLib:SendAddonMessage("{BULK||NORMAL||ALERT}", "prefix", "text", "bnetIDAccount"')
	end
	if callbackFn and type(callbackFn)~="function" then
		error('ChatThrottleLib:SendAddonMessage(): callbackFn: expected function, got '..type(callbackFn), 2)
	end
	...
	if RegisterAddonMessagePrefix then
		...
		nSize = nSize + prefix:len() + 1
		if nSize>255 then
			error("ChatThrottleLib:BNSendGameData(): prefix + message length cannot exceed 254 bytes", 2)
		end
	end
